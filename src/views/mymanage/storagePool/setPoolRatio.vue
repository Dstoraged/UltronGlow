          ...
          <div class="row" style="padding: 6px; text-align: left">
            <div class="col-md-4">
              <h4
                ...
                style="width: 100%; text-align: left"
              >
                {{ $t("storagePool.setupRecord") }}
              </h4>
            </div>
          </div>
          <txns-records
            :address="$store.getters.account"
            :label="$t('storagePool.SPRewardRatio')"
            tx-type="spetrate"
          />
        </div>
      </div>
    ...
    loadData() {
      ...
      api
        ...
        .then((response) => {
          ...
          this.ruleForm.ratio =
            this.datas.entrustRate < 0 ? 0 : this.datas.entrustRate;
        })
        .catch((error) => {
          console.error(error);
        });
      ...
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          ...
          if (reObj) {
            ...
            let _this_ = this;
            let setData = {
              title: `${this.$t("storagePool.SPRewardDistributionRatio")}`,
              context: ` ${this.$t(
                "storagePool.storagePoolAddress"
              )}: ${_this_.getAddrFormat(_this_.datas.hash)}<br/>${this.$t(
                "storagePool.SPRewardRatio"
              )}：${this.ruleForm.ratio} %`,
              request: null,
              closebtnshow: false,
              closewin(type) {
