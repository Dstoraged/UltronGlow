    ...
    <div class="card" style="margin-top: 40px">
      <div class="card-body">
        <div class="row" style="padding: 6px; text-align: left">
          ...
          <div class="col-md-4 i-ma-sm" style="text-align: right">
            <el-button type="primary" @click="toAdd">{{ $t("storagePool.createSP") }}</el-button>
          </div>
        </div>
        <h-tbale ref="htable" :show-subtitle="false" :table-id="tableId" :table-heard="tableHeard" :table-data="tableData"
              ...
              <div class="filter-win" v-show="searchs.win.status">
                <div>
                  <select class="form-control" v-model="searchs.data.status">
                    ...
                    <option value="1+0">{{ `${$t("market.status0")} + ${$t("market.status7")}` }}</option>
          ...
          <div slot="rightCol" slot-scope="scope" class="width-10">
            <el-button :disabled="scope.data.activeStatus !== 1" size="mini" type="text"
              @click="goToUrl('mymanage/storagePoolPledgeEN', scope.data.hash)">
              {{ $t("storageNode.trustpledge") }}
            </el-button>
          </div>
        </h-tbale>
      </div>
...
export default {
  ...
  data() {
    ...
    return {
      tableId: "storagePool_public_Index",
      searchs: {
        data: {
          query: "",
          status: "1+0",
      ...
      tableHeard: [
        ...
        },
        {
          ...
          valMap(val) {
            switch (`${val}`) {
              case "0":
                return `<span style='color:#555'>${_this_.$t("market.status7")}</span>`;
              case "1":
                return `<span class='text-green'>${_this_.$t("market.status0")}</span>`;
              case "2":
                return `<span style='color:#aaa'>${_this_.$t("comm.exited")}</span>`;
            }
          },
        },
        {
  ...
  methods: {
    ...
    toAdd() {
      this.$router.push("/mymanage/storagePoolAdd");
    },
    formatTime(time) {
