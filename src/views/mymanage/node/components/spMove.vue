    ...
    <div class="card" style="margin-top: 40px">
      <div class="card-body">
        ...
        >
          ...
          <el-form-item :label="$t('nodeMove.theCurrentSP')">
            {{ datas.node_address }}
          </el-form-item>
          ...
          <!--<template v-if="selectItem">
            <el-form-item :label="$t('nodeMove.SPRewardRatio')">
              {{ `${selectItem.entrustRate || 0} %` }}
            </el-form-item>

            <el-form-item :label="$t('nodeMove.SPSpaceTotal')">
              {{ bytesToSize(selectItem.totalCapacity) }}
            </el-form-item>

            <el-form-item :label="$t('nodeMove.SPSpace')">
              {{ bytesToSize(selectItem.usedCapacity) }}
            </el-form-item>
          </template>-->
...
import selectNode from "./selectSpNode.vue";
...
export default {
  ...
  methods: {
      ...
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          ...
          if (sdk) {
            let _this_ = this;
            let setData = {
              title: this.$t(`nodeMove.TransferPledge`),
              context: ``,
              request: null,
              closebtnshow: false,
              closewin(type) {
    ...
    onSelect(item,heards) {
      this.ruleForm.newAddr = item.hash;
