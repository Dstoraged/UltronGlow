  ...
  <div class="container-fluid">
    <div class="card" style="margin-top: 40px">
      <div class="card-body">
        ...
        <el-form v-if="isChange" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="auto"
          class="center-form max-width-10" v-loading="loading">
          ...
          <el-form-item :label="$t('nodeMove.goalNode')" prop="newAddr">
            <!--<f-table-tools :heards="selectheard" :data-item="selectItem"></f-table-tools>-->
            {{ ruleForm.newAddr }}
          ...
          :notHeards="['node_address','node_type']"
         ...
         <!-- <template v-if="selectItem">
            <el-form-item :label="$t('nodeMove.nodeRewardRatio')">
              {{ `${selectItem.rate > 0 ? selectItem.rate / 100 : 0} %` }}
            </el-form-item>
          </template>-->
    ...
    <select-node :now-addr="datas.node_address" v-model="selectWin" v-if="selectWin" @onSub="onSelect"></select-node>
  </div>
</template>
<script>
...
import selectNode from "./selectPosNode.vue";
import { ethers } from "ethers";

import fTableTools from "@/components/modules/fTableTools.vue";
import fTableCels from "@/components/modules/fTableCels.vue";

export default {
  name: "posMove",
  components: { selectNode, fTableTools,fTableCels },
  ...
  methods: {
      ...
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          ...
          if (sdk) {
            ...
            let setData = {
              title: `${this.pledge_type} ${_this_.$t("nodeMove.transfer")}`,
    ...
    onSelect(item, heards) {
      this.ruleForm.newAddr = item.node_address;
