...
export default {
  ...
  methods: {
    ...
    walletSelect() {
      return utils.walletSelect({
        _this_: this,
        addr: this.pledgeData.deviceAddr,
        msg: "mymanageMining.pleaseSNAddr",
      });
    },
    ...
    submitForm() {
      this.$refs["ie_form"].validate((valid) => {
        if (valid) {
          ...
          let title = this.btnText;
          let context = "";
          let device_address = this.pledgeData.deviceAddr;
          let newManageAddr = webSdk.sdkUtils.hashToUX(
          ...
          if (this.isUp) {
            title = this.btnText;
            context = `${this.$t("storagePool.newAdmin")} :${newManageAddr}`;
          }
          newManageAddr = newManageAddr.toLocaleLowerCase();
  ...
  computed: {
    ...
    btnText() {
      return this.isUp ? this.$t("storagePool.AdminTransfer") :  this.$t("storagePool.bindingAdmin");
    },
    originalLabel() {
      return this.isUp ? this.$t("storagePool.originalAdmin") : this.$t("storagePool.bindingAdmin");
    },
    newLabel() {
      return this.isUp ? this.$t("storagePool.newAdmin") : this.$t("storagePool.bindingAdmin") ;
    },
    isUp() {
      return !!this.pledgeData.managerAddr;
    },
