      ...
      <div class="card-body">
          ...
          <div class="col-md-9">
            <div class="input-group" style="max-width: 500px; text-align: left">
              ...
              <input
                type="text"
                class="form-control"
                :placeholder="$t('storagerental.searchHashAddress')"
                v-model="searchInput"
        ...
        <template slot="table_cell_rentStatus" slot-scope="scope">
          ...
          <template slot="rightCol" slot-scope="scope">  
              <el-button
              type="text"
              :disabled="!(scope.data.rentStatus == 1)"
              size="mini"
              @click="goToUrlConfig(scope.data)"
              class="equal-width"
              >{{ $t("storagerental.rentalconfig") }}</el-button>
          </template>
        </h-tbale>
      </div>
    </div>
  ...
  data() {
    ...
    return {
      tableHeard: [
        ...
        {
          title: "storagerental.revenuesaddress",
          name: "revenue_addr",
          format: true,
          popover: true,
          isClick: true,
  ...
  methods: {
      ...
      api
        ...
        .then((re) => {
          ...
          if(!this.shareItem.isConfig){
            console.log("rentHash="+data.rentHash)
    ...
    async loadTable(query, toTop = true, pageNum) {
      ...
      var params = {
        ...
        size: query.pageSize,
        revenue_addr: this.$store.getters.account,
        status:1
      };
