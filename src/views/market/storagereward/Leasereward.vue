  ...
  <div>
    <h4 class="mt-0 header-title" style="width: 100%; text-align: left">
      <p class="sub-title" style="width: 100%; text-align: left">
        {{ $t("storagereward.totalnumber", [totall]) }}
      </p>
    </h4>
    <div class="table-responsive" style="min-height: 200px">
      <table class="table table-hover mb-0">
        ...
        <tbody>
          <tr v-for="(item, index) in LeaserewardList" :key="index">
            <td>{{ item.ranking }}</td>
            <td v-if="devflag === 0">
          ...
          <li class="page-item">
            <a
              ...
              style="
                ...
                color: #41c0cb;
              "
              @click="getLeaserewardList(pagel - 1)"
            >
          ...
          <li class="page-item">
            <a
              ...
              style="
                ...
                color: #41c0cb;
              "
              @click="getLeaserewardList(totalPagel)"
              >{{ $t("pagination.last") }}</a
            >
          </li>
        </ul>
...
export default {
  ...
  created() {
    if (sessionStorage.getItem("detail")) {
      if (typeof sessionStorage.getItem("currentPagel") !== "undefined") {
        this.pagel = Number(sessionStorage.getItem("currentPagel") || 1);
      }
      if (typeof sessionStorage.getItem("pageSizel") !== "undefined") {
        this.pageSizel = Number(sessionStorage.getItem("pageSizel") || 10);
      }
      sessionStorage.removeItem("detail");
    }

    this.getLeaserewardList(this.pagel);
  },
  methods: {
    formatTime(time) {
    ...
    handleSizeChangel() {
      var val = $("#selectPagel").val();
      this.pageSizel = val;
      this.getLeaserewardList(1);
      sessionStorage.setItem("pageSizel", val);
    },

    handleCurrentChangel(val) {
      this.pagel = val;
      this.getLeaserewardList(this.page);
      sessionStorage.setItem("currentPagel", val);
    },

    seachData() {
      this.LeaserewardList = [];
      this.getLeaserewardList(1);
    },

    async getLeaserewardList(page) {
      showLoading();
      
      if (page < 1 || (this.totalPagel > 0 && page > this.totalPagel)) {
        hideLoading();
        return;
      }

      this.pagel = page;
      sessionStorage.setItem("currentPagel", page);
      let param = {
        current: this.pagel,
        size: this.pageSizel,
        revenueAddress: null,
      ...
      api
        ...
        .then((response) => {
          if (response.statusCode !== 0) {
            ...
            if (data.length < this.totall) {
              this.loadText = this.$t("comm.bottomLine");
            }
            this.totall = response.result.total;
            this.totalPagel = response.result.pages;

            const rand = (this.pagel - 1) * this.pageSizel;
            const newData = [];
            var temp = {};
