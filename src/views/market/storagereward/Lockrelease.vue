  ...
  <div>
    <h4 class="mt-0 header-title" style="width: 100%; text-align: left">
      <p class="sub-title" style="width: 100%; text-align: left">
        {{ $t("storagereward.totalnumber", [totalr]) }}
      </p>
    </h4>
    <div class="table-responsive" style="min-height: 200px">
      <table class="table table-hover mb-0">
        ...
        <tbody>
          <tr v-for="(item, index) in LockreleaseList" :key="index">
            <td>{{ item.ranking }}</td>
            <!--              
                <td v-if="devflag===0">
          ...
          <li class="page-item">
            <a
              ...
              style="
                ...
                color: #41c0cb;
              "
              @click="getLockreleaseList(pager - 1)"
            >
          ...
          <li class="page-item">
            <a
              ...
              style="
                ...
                color: #41c0cb;
              "
              @click="getLockreleaseList(totalPager)"
              >{{ $t("pagination.last") }}</a
            >
          </li>
        </ul>
...
export default {
  ...
  created() {
    if (sessionStorage.getItem("detail")) {
      if (typeof sessionStorage.getItem("currentPager") !== "undefined") {
        this.pager = Number(sessionStorage.getItem("currentPager") || 1);
      }
      if (typeof sessionStorage.getItem("pageSizer") !== "undefined") {
        this.pageSizer = Number(sessionStorage.getItem("pageSizer") || 10);
      }
      sessionStorage.removeItem("detail");
    }

    this.getLockreleaseList(this.pager);
  },
  methods: {
    formatTime(time) {
    ...
    handleSizeChanger() {
      ...
      sessionStorage.setItem("pageSizer", val);
    },

    handleCurrentChanger(val) {
      this.pager = val;
      this.getLockreleaseList(this.pager);
      sessionStorage.setItem("currentPager", val);
    },

    seachData() {
      this.LockreleaseList = [];
      this.getLockreleaseList(1);
    },

    async getLockreleaseList(page) {
      showLoading();
      ...
      this.pager = page;
      sessionStorage.setItem("currentPager", page);
      let param = {
        current: this.pager,
        size: this.pageSizer,
        revenueAddress: null,
