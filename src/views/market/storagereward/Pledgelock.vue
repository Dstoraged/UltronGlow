  ...
  <div>
    <h4 class="mt-0 header-title" style="width: 100%; text-align: left">
      <p class="sub-title" style="width: 100%; text-align: left">
        {{ $t("storagereward.totalnumber", [totalp]) }}
      </p>
    </h4>
    <div class="table-responsive" style="min-height: 200px">
      <table class="table table-hover mb-0">
        ...
        <tbody>
          <tr v-for="(item, index) in PledgelockList" :key="index">
            <td>{{ item.ranking }}</td>
            <td v-if="devflag === 0">
          ...
          <li class="page-item">
            <a
              ...
              style="
                ...
                color: #41c0cb;
              "
              @click="getPledgelockList(pagep - 1)"
            >
          ...
          <li class="page-item">
            <a
              ...
              style="
                ...
                color: #41c0cb;
              "
              @click="getPledgelockList(totalPagep)"
              >{{ $t("pagination.last") }}</a
            >
          </li>
        </ul>
...
export default {
  ...
  created() {
    if (sessionStorage.getItem("detail")) {
      if (typeof sessionStorage.getItem("currentPagep") !== "undefined") {
        this.pagep = Number(sessionStorage.getItem("currentPagep") || 1);
      }
      if (typeof sessionStorage.getItem("pageSizep") !== "undefined") {
        this.pageSizep = Number(sessionStorage.getItem("pageSizep") || 10);
      }
      sessionStorage.removeItem("detail");
    }

    this.getPledgelockList(this.pagep);
  },
  methods: {
    formatTime(time) {
    ...
    handleSizeChangep() {
      var val = $("#selectPagep").val();
      this.pageSizep = val;
      this.getPledgelockList(1);
      sessionStorage.setItem("pageSizep", val);
    },

    handleCurrentChangep(val) {
      this.pagep = val;
      this.getPledgelockList(this.pagep);
      sessionStorage.setItem("currentPagep", val);
    },
    ...
    async getPledgelockList(page) {
      showLoading();
      await utils.getNowHeight();
      if (page < 1 || (this.totalPagep > 0 && page > this.totalPagep)) {
        hideLoading();
        return;
      }
      this.pagep = page;
      sessionStorage.setItem("currentPagep", page);
      let param = {
        current: this.pagep,
        size: this.pageSizep,
        revenueAddress: null,
      ...
      api
        ...
        .then((response) => {
          if (response.statusCode !== 0) {
            ...
            if (data.length < this.totalp) {
              this.loadText = this.$t("comm.bottomLine");
            }
            this.totalp = response.result.total;
            this.totalPagep = response.result.pages;

            const rand = (this.pagep - 1) * this.pageSizep;
            const newData = [];
            var temp = {};
