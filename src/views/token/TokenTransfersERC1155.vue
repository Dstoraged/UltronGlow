    ...
    <div class="card" style="margin-top: 40px">
      <div class="card-body">
        <h4 class="mt-0 header-title" style="width: 100%; text-align: left">
          {{ $t("tokentransfers.title1155", ['URC-1155']) }}
        </h4>
        <p class="sub-title" style="width: 100%; text-align: left">
          {{ $t("tokentransfers.currentTranNumber", [total]) }}
        </p>
        <h-tbale ref="htableTokenTransfersERC1155" :table-heard="tableHeard" :table-data="tableData"
          :show-right-tools="false" tableId="TokenTransfersERC1155tableId" :showSubtitle="false" @inint-ok="tableInit"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      /*
        {
          title: "tokentransfers.value",
          name: "amount",
          valMap(val,item) {
            return (
              utils.weedZero(helper.getDataFormat(val, item.decimals, 10000), 8)
            );
          },
          more: true,
        },   
  ...
  methods: {
    ...
    loadTable(query, toTop = true, pageNum) {
      if (!query && this.$refs.htableTokenTransfersERC1155) {
        query = this.$refs.htableTokenTransfersERC1155.tableQuery;
      }
