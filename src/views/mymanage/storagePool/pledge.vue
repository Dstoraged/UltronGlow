    ...
    <div class="card" style="margin-top: 40px">
      <div class="card-body">
        ...
        >
          ...
          <el-form-item :label="$t('storagePool.maxSpace')">
            {{ bytesToSize(datas.totalCapacity) }}
          </el-form-item>
          <el-form-item :label="$t('storagePool.TotalAmountPledged')">
            {{ numUtg(datas.pledgeAmount || 0) }}</el-form-item
          >
          <el-form-item :label="$t('storagePool.pledgedAmount')">
            {{ numUtg(datas.havAmount || 0) }}
          </el-form-item>
        ...
        amount: [
          { message: _this_.$t("messages.required"), required: true },
          {
            validator: (rule, value, callback) => {
              ...
              let min = 1;
              if (!_this_.datas.managerAmount) {
                min = 625;
              }
              if (value < min) {
                callback(new Error(`>=${min}`));
              } else {
                callback();
              }
            },
  ...
  methods: {
    loadData() {
      ...
      api
        ...
        .then((response) => {
          ...
          this.ruleForm.amount = this.awaitNum < 0 ? 0 : this.awaitNum;
        })
        .catch((error) => {
          console.error(error);
        });
    ...
    submitForm(formName) {
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          let addr=this.datas.managerAddr;
          let msg="messages.pleaseManageAddr"
            if(!this.datas.managerAmount){
                addr=this.datas.revenueAddr||addr
                msg="messages.pleaserevenues"
            }
          ...
          if (reObj) {
            ...
            let _this_ = this;
            let setData = {
              title: `${_this_.$t("Transactions.spchpg")}`,
              context: ` ${_this_.$t(
